(function () {
  ...
  const siteKey = currentScript.getAttribute('data-a11y-bridge');
  if (!siteKey) {
    return;
  }

  const platformOrigin = new URL(currentScript.src, window.location.href).origin;
  const storageKey = 'a11yBridgePrefs:' + siteKey;
    ...
    if (config.widget.showFontScale) {
      controls.appendChild(
        createControlRow(
          ...
          [
            { label: 'A-', onClick: () => updatePrefs({ fontScale: 'small' }) },
            { label: 'A', onClick: () => updatePrefs({ fontScale: 'normal' }) },
            { label: 'A+', onClick: () => updatePrefs({ fontScale: 'large' }) },
          ],
        ),
      );
    }
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      .ab-widget-shell.ab-bottom-left {
        left: 20px;
        right: auto;
        align-items: flex-start;
      }
      .ab-widget-button {
        --ab-widget-color: #0f6a73;
        min-height: 52px;
        padding: 0 18px;
        border: 0;
        border-radius: 999px;
        background: var(--ab-widget-color);
        color: #fff;
        font-weight: 700;
        box-shadow: 0 14px 34px rgba(17, 37, 51, 0.18);
        cursor: pointer;
      }
      .ab-compact .ab-widget-button {
        min-height: 44px;
        padding: 0 14px;
        font-size: 14px;
      }
      .ab-large .ab-widget-button {
        min-height: 58px;
        padding: 0 22px;
        font-size: 18px;
      }
      .ab-widget-panel {
        ...
        background: #fffdf8;
        color: #132838;
        border: 1px solid rgba(19, 40, 56, 0.1);
        box-shadow: 0 20px 60px rgba(17, 37, 51, 0.16);
        padding: 18px;
      }
      .ab-widget-title {
        margin: 0;
      ...
      .ab-widget-toggle {
        ...
        padding: 0 10px;
        border-radius: 999px;
        border: 1px solid rgba(19, 40, 56, 0.12);
        background: #fff;
        color: #132838;
        cursor: pointer;
      }
      ...
      html.ab-pref-contrast {
        filter: contrast(1.18);
      }
      html.ab-pref-underline a {
        text-decoration: underline !important;
      }
      html.ab-pref-reduce-motion *,
      html.ab-pref-reduce-motion *::before,
      html.ab-pref-reduce-motion *::after {
      ...
      .ab-widget-button:focus-visible,
      .ab-widget-action:focus-visible,
      .ab-widget-toggle:focus-visible,
      .ab-widget-link:focus-visible {
        outline: 3px solid rgba(15, 106, 115, 0.28);
        outline-offset: 3px;
      }
    `;
  ...
  function updatePrefs(partial) {
    const nextPrefs = Object.assign({}, loadPrefs(), partial);
    savePrefs(nextPrefs);
