(function () {
  ...
  function renderWidget(config) {
    ...
    buttonIcon.textContent = 'A';

    var buttonLabel = document.createElement('span');
    buttonLabel.className = 'ab-widget-button-label';
    buttonLabel.textContent = config.widget.label || 'Accessibility';

    var buttonHint = document.createElement('span');
    buttonHint.className = 'ab-widget-button-hint';
    buttonHint.textContent = config.widget.language === 'en' ? 'Open panel' : 'פתח פאנל';

    button.appendChild(buttonGlow);
    button.appendChild(buttonIcon);
    button.appendChild(buttonLabel);
    button.appendChild(buttonHint);
    ...
    description.textContent =
      config.widget.language === 'en'
        ? 'Personal display preferences with direct access to the accessibility statement.'
        : 'העדפות תצוגה אישיות עם גישה ישירה להצהרת הנגישות.';
  ...
  function injectStyles() {
    ...
    style.textContent = ''
      + '.ab-widget-shell{position:fixed;z-index:999999;bottom:20px;right:20px;display:flex;flex-direction:column-reverse;gap:14px;align-items:flex-end;font-family:Inter,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;}'
      + '.ab-widget-shell.ab-bottom-left{left:20px;right:auto;align-items:flex-start;}'
      + '.ab-widget-button{--ab-widget-color:#0f6a73;position:relative;display:inline-flex;align-items:center;gap:12px;min-height:60px;padding:0 18px 0 14px;border:0;border-radius:999px;background:linear-gradient(135deg,var(--ab-widget-color),#16333a 72%);color:#fff;font-weight:700;box-shadow:0 18px 46px rgba(8,22,31,.28);cursor:pointer;overflow:hidden;backdrop-filter:blur(10px);}'
      ...
      + '.ab-widget-button-icon{position:relative;width:34px;height:34px;display:inline-grid;place-items:center;border-radius:50%;background:rgba(255,255,255,.18);border:1px solid rgba(255,255,255,.18);font-size:14px;font-weight:900;letter-spacing:.08em;}'
      + '.ab-widget-button-label,.ab-widget-button-hint{position:relative;}'
      + '.ab-widget-button-label{display:block;font-size:15px;}'
      + '.ab-widget-button-hint{display:block;font-size:11px;opacity:.76;font-weight:500;}'
      ...
      + '.ab-widget-panel{width:min(360px,calc(100vw - 28px));border-radius:28px;background:linear-gradient(180deg,rgba(255,251,243,.98),rgba(248,240,227,.98));color:#16303a;border:1px solid rgba(22,48,58,.1);box-shadow:0 30px 80px rgba(10,24,32,.22);padding:18px;position:relative;overflow:hidden;}'
      ...
      + '.ab-widget-brand-badge{width:42px;height:42px;display:inline-grid;place-items:center;border-radius:16px;background:linear-gradient(135deg,#173941,#0f6a73);color:#fff;font-weight:800;box-shadow:0 10px 24px rgba(15,106,115,.22);}'
      ...
      + '.ab-widget-close{width:36px;height:36px;border:1px solid rgba(22,48,58,.1);border-radius:14px;background:rgba(255,255,255,.72);color:#16303a;font-size:26px;line-height:1;cursor:pointer;}'
      ...
      + '.ab-widget-row{align-items:flex-start;justify-content:space-between;gap:12px;padding:14px;border-radius:20px;background:rgba(255,255,255,.74);border:1px solid rgba(22,48,58,.08);box-shadow:inset 0 1px 0 rgba(255,255,255,.6);}'
      ...
      + '.ab-widget-action,.ab-widget-toggle{min-height:38px;padding:0 12px;border-radius:999px;border:1px solid rgba(22,48,58,.12);background:#fff;color:#16303a;cursor:pointer;font-weight:700;box-shadow:0 6px 16px rgba(14,31,40,.08);}'
      ...
      + '.ab-widget-link{display:inline-flex;align-items:center;justify-content:center;min-height:42px;padding:0 14px;border-radius:14px;background:#fff;color:#0f6a73;font-weight:700;text-decoration:none;border:1px solid rgba(15,106,115,.14);width:max-content;}'
